...
| `Boss/Temp/TempPattern_Aimed.asset` · `TempPattern_Ring.asset` | `AimedPatternSO` · `RingPatternSO` | 임시 보스 패턴 (셋업 메뉴가 생성·덮어씀) |
| `Boss/Temp/TempSelector_Sequential.asset` | `SequentialPatternSelectorSO` | 순차 선택 전략 |
| `Boss/Temp/TempPhase_0.asset` · `TempEncounter.asset` | `BossPhaseDataSO` · `BossEncounterDataSO` | 임시 페이즈 1개 / 인카운터 |
| `Boss/Temp/TempCombatScene.asset` | `CombatScene` | 전투 단위 (BootProfile 입력) |
| `Game/BootProfile_CombatOnly.asset` | `BootProfile` | 전투 단독 부팅 프로필 |
...
## 미작성 스펙 (TODO)
- `BossDataSO` / `BossEncounterDataSO` / `BossPhaseDataSO`(`Patterns[]`: pattern·weight·delayAfter, `Selector`) 필드 의미와 작성 규칙
- ⚠️ `Boss/Temp/` 에셋은 `Tools/Unorder/Test/Setup Temp Boss` 실행 시 값이 **덮어써진다**. 손으로 튜닝할 거면 복사본을 쓸 것
